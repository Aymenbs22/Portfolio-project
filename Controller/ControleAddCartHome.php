...
if(isset($_GET['ref'])) {
     ...
     //We Can use val or ref
      $req = $DB->db->prepare('SELECT reference FROM products WHERE reference=:ref', array('ref'=> $_GET['ref']));
      $req->execute(array('ref'=> $_GET['ref']));
      $products = $req->fetchAll(); //it give us the reference of selected product
